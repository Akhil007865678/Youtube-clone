...
const Search = ({ sideNavbar }) => {
  const [filter, setFilter] = useState([]); // All videos fetched from API
  const [videos, setVideos] = useState([]); // Filtered videos based on search
  const { id } = useParams(); // Search query from route
  ...
  const fetchVideo = async () => {
    try {
      ...
      setVideos(response.data.videos); // Initialize with all videos
    } catch (error) {
  ...
  useEffect(() => {
    if (!id) {
      setVideos(filter); // Show all videos if no search query
    } else {
