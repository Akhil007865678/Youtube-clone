...
const Search = () => {
    const [videos, setVideos] = useState([]); // Initialize as an empty array to store video data

    // Function to fetch videos from the backend
    const fetchVideo = async () => {
        try {
            const response = await axios.get(`http://localhost:3000/api/allvideo`);
            setVideos(response.data.videos); // Assuming the API returns a `videos` array
        } catch (error) {
