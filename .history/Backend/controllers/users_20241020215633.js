...
const cookieOptions = {
    ...
    sameSite: 'Lax'  // Corrected the typo here
};
...
const signIn = async (req, res) => {
    try{
        ...
        if(user && await bcrypt.compare(password, user.password)){
            const token = jwt.sign({userId: user._id}, 'Key', cookieOptions);
            res.cookie('token', token, cookieOptions);  // Pass cookieOptions here
